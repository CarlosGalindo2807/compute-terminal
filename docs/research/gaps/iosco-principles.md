...
2026-06-25 (P7 + P8 rows + queue updated after the self-classification proposal landed).
...
| P7 | Data sufficiency | partial / **structurally weak** | All inputs anchored in a genuine arms-length cash market for GPU-hours; inputs are **firm executable list prices** (closer to BMR "committed quotes" than to indicative submissions). 24h rolling window, ≥ 6 providers when fully ramped. `invoice_observations` table exists (migration 011) as the latent transaction layer — empty today. | Inputs are executable **listings**, not observed trades. Strict P7 reading requires "anchored by observable transactions". On-demand compute has no public consolidated tape. Path now mapped (see note): **Track A** — self-classify as a *published-quote benchmark* + publish data-input hierarchy on `/methodology` (proposal landed 2026-06-25 — [`proposals/2026-06-25-methodology-published-quote-self-classification.md`](../proposals/2026-06-25-methodology-published-quote-self-classification.md); awaiting Committee deliberation, target effective date 2026-07-25). **Track B** — stand up `invoice_observations` ingest, then publish a list-price-vs-observed-effective-price reconciliation report (P1, infra, no hard-limit file). **Track C** — provider-count-scaled quorum so thin-GPU indices suppress earlier (P1, methodology-class → proposal + committee). | P0 (Track A) / P1 (B, C) | Note done: [`notes/2026-05-12-listings-vs-transactions-iosco-p7.md`](../notes/2026-05-12-listings-vs-transactions-iosco-p7.md). Proposal open: [`proposals/2026-06-25-methodology-published-quote-self-classification.md`](../proposals/2026-06-25-methodology-published-quote-self-classification.md). On merge of the implementing PR + effective date, row moves to `partial → compliant in design`. |
| P8 | Hierarchy of data inputs | partial | Hierarchy exists in code (rule → alias → fuzzy → Claude ≥ 0.95 auto → Claude 0.70–0.95 admin queue → outlier check → eligibility check → VWAP). Zero expert judgment in published-number path. | Hierarchy is implicit, not published. Proposal open: the three-rank input hierarchy (observed transactions / firm executable quotes / indicative submissions, "not used and not contemplated") ships on `/methodology` in the same edit as the P7 self-classification — see [`proposals/2026-06-25-methodology-published-quote-self-classification.md`](../proposals/2026-06-25-methodology-published-quote-self-classification.md). | P1 | On merge of the implementing PR + effective date, row moves to `compliant in design, hierarchy published`. See [`notes/2026-05-12-listings-vs-transactions-iosco-p7.md`](../notes/2026-05-12-listings-vs-transactions-iosco-p7.md) §4–5. |
...
4. P7 + P8 — Proposal landed 2026-06-25 ([`proposals/2026-06-25-methodology-published-quote-self-classification.md`](../proposals/2026-06-25-methodology-published-quote-self-classification.md)). Awaiting @CarlosGalindo2807 deliberation. On approval + 30-day public notice, implementing PR ships v1.0 → v1.0.1 docs-only bump (formula unchanged); effective date 2026-07-25. *This is also the dry-run of the methodology change procedure (gap-matrix queue item 10, P12).*
...
- **2026-06-25** — P7 + P8 rows updated to point at [`proposals/2026-06-25-methodology-published-quote-self-classification.md`](../proposals/2026-06-25-methodology-published-quote-self-classification.md) (Track A, P0). Proposal targets v1.0 → v1.0.1 docs-only bump on `/methodology` with no formula change. Priority-queue P0 item 4 updated from "write the proposal" to "awaiting Committee deliberation"; implementing PR + effective date 2026-07-25 still ahead. Tracks B (invoice ingest + reconciliation) and C (scaled quorum) remain open at P1. (index-architect fourth run.)
