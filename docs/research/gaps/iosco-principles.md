...
2026-09-07 (P0 queue item 4 promoted from "next: write the proposal" to
"proposal drafted — pending Committee review": see
[`proposals/2026-09-07-methodology-page-input-classification-hierarchy.md`](../proposals/2026-09-07-methodology-page-input-classification-hierarchy.md)).
...
| P7 | Data sufficiency | partial / **structurally weak** | All inputs anchored in a genuine arms-length cash market for GPU-hours; inputs are **firm executable list prices** (closer to BMR "committed quotes" than to indicative submissions). 24h rolling window, ≥ 6 providers when fully ramped. `invoice_observations` table exists (migration 011) as the latent transaction layer — empty today. | Inputs are executable **listings**, not observed trades. Strict P7 reading requires "anchored by observable transactions". On-demand compute has no public consolidated tape. Path now mapped (see note): **Track A** — self-classify as a *published-quote benchmark* + publish data-input hierarchy on `/methodology` (**proposal drafted 2026-09-07** → [`proposals/2026-09-07-methodology-page-input-classification-hierarchy.md`](../proposals/2026-09-07-methodology-page-input-classification-hierarchy.md); pending @CarlosGalindo2807 review). **Track B** — stand up `invoice_observations` ingest, then publish a list-price-vs-observed-effective-price reconciliation report (P1, infra, no hard-limit file). **Track C** — provider-count-scaled quorum so thin-GPU indices suppress earlier (P1, methodology-class → proposal + committee). | P0 (Track A — proposal pending review) / P1 (B, C) | Proposal drafted: [`proposals/2026-09-07-methodology-page-input-classification-hierarchy.md`](../proposals/2026-09-07-methodology-page-input-classification-hierarchy.md). On merge: downgrade this row's disclosure-gap portion to closed and priority to `P1` (Track B remains). |
| P8 | Hierarchy of data inputs | partial | Hierarchy exists in code (rule → alias → fuzzy → Claude ≥ 0.95 auto → Claude 0.70–0.95 admin queue → outlier check → eligibility check → VWAP). Zero expert judgment in published-number path. | Hierarchy is implicit, not published. | P1 (proposal drafted) | Add "Hierarchy of data inputs" subsection to `/methodology` listing each ingestion stage and the deterministic rule — **bundled in the P7 self-classification proposal** ([`proposals/2026-09-07-methodology-page-input-classification-hierarchy.md`](../proposals/2026-09-07-methodology-page-input-classification-hierarchy.md), pending review). On merge: mark this row `compliant`. |
...
4. P7 + P8 — Proposal for the `/methodology` "published-quote benchmark" self-classification + data-input hierarchy subsection. *Research done ([note](../notes/2026-05-12-listings-vs-transactions-iosco-p7.md)); **proposal drafted 2026-09-07** → [`proposals/2026-09-07-methodology-page-input-classification-hierarchy.md`](../proposals/2026-09-07-methodology-page-input-classification-hierarchy.md). **Pending @CarlosGalindo2807 review; then a follow-up PR applies the JSX to `apps/web/app/methodology/page.tsx`.** Closes two quality-pillar gaps in one edit.*
...
- **2026-09-07** — P0 queue item 4 promoted from "next: write the proposal" to "proposal drafted — pending Committee review". Proposal is [`proposals/2026-09-07-methodology-page-input-classification-hierarchy.md`](../proposals/2026-09-07-methodology-page-input-classification-hierarchy.md); it edits `apps/web/app/methodology/page.tsx` (hard-limit) with two new sections and does **not** touch `PUBLISHED_METHODOLOGY`. Rows P7 and P8 updated to point at the proposal; both note "on merge" transitions (P7 disclosure gap closes → downgraded to P1; P8 becomes `compliant`). Neither row is closed yet — the code edit is a separate follow-up PR gated on this proposal's approval. (index-architect fourth run.)
