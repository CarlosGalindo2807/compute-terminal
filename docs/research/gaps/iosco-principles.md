...
2026-08-10 (P7 + P8 rows re-pointed at proposal
[`proposals/2026-08-10-methodology-published-quote-classification-and-hierarchy.md`](../proposals/2026-08-10-methodology-published-quote-classification-and-hierarchy.md);
P0 queue item 4 flipped from "write the proposal" to "review and merge the
proposal, then cut the follow-up `/methodology` PR").
...
| P7 | Data sufficiency | partial / **structurally weak** (Track A awaiting merge) | All inputs anchored in a genuine arms-length cash market for GPU-hours; inputs are **firm executable list prices** (closer to BMR "committed quotes" than to indicative submissions). 24h rolling window, ≥ 6 providers when fully ramped. `invoice_observations` table exists (migration 011) as the latent transaction layer — empty today. | Inputs are executable **listings**, not observed trades. Strict P7 reading requires "anchored by observable transactions". On-demand compute has no public consolidated tape. Three-track response mapped: **Track A** — self-classify as a *published-quote benchmark* + publish data-input hierarchy on `/methodology`: **proposal drafted 2026-08-10** ([`proposals/2026-08-10-methodology-published-quote-classification-and-hierarchy.md`](../proposals/2026-08-10-methodology-published-quote-classification-and-hierarchy.md)), awaiting @CarlosGalindo2807 review; when merged, cuts a follow-up PR editing `/methodology` and closes P8 alongside. **Track B** — stand up `invoice_observations` ingest, then publish a list-price-vs-observed-effective-price reconciliation report (P1, infra, no hard-limit file). **Track C** — provider-count-scaled quorum so thin-GPU indices suppress earlier (P1, methodology-class → proposal + committee). | P0 (Track A — awaiting merge) / P1 (B, C) | Proposal open: [`proposals/2026-08-10-methodology-published-quote-classification-and-hierarchy.md`](../proposals/2026-08-10-methodology-published-quote-classification-and-hierarchy.md). Source note: [`notes/2026-05-12-listings-vs-transactions-iosco-p7.md`](../notes/2026-05-12-listings-vs-transactions-iosco-p7.md). |
| P8 | Hierarchy of data inputs | partial (proposal awaiting merge) | Hierarchy exists in code (rule → alias → fuzzy → Claude ≥ 0.95 auto → Claude 0.70–0.95 admin queue → outlier check → eligibility check → VWAP). Zero expert judgment in published-number path. | Hierarchy is implicit, not published. Bundled with P7 Track A in [`proposals/2026-08-10-methodology-published-quote-classification-and-hierarchy.md`](../proposals/2026-08-10-methodology-published-quote-classification-and-hierarchy.md) — the proposal's "Hierarchy of data inputs" and "Determination pipeline" subsections are the exact `/methodology` copy that closes this row on the follow-up implementation PR. | P1 (now blocked on the same P0 proposal as P7 Track A) | On merge of the follow-up `/methodology` PR: flip status to `compliant`, point evidence at the new page section, remove from queue. |
...
4. P7 + P8 — Proposal for the `/methodology` "published-quote benchmark" self-classification + data-input hierarchy subsection. **Proposal drafted 2026-08-10** ([`proposals/2026-08-10-methodology-published-quote-classification-and-hierarchy.md`](../proposals/2026-08-10-methodology-published-quote-classification-and-hierarchy.md)) — the proposal contains verbatim JSX for the `/methodology` insert, an argued position on whether the 30-day-notice procedure applies to disclosure-only edits (author's position: it does not; Committee to decide), and a fallback rollout plan if the Committee reads the notice period more broadly. *Next step: @CarlosGalindo2807 reviews and merges the proposal PR (docs-only), then a follow-up PR cuts the actual `/methodology` edit.*
...
7. P8 — Hierarchy-of-data-inputs subsection on `/methodology` (bundled with P0 item 4 — same proposal, same page edit; awaiting merge).
...
- **2026-08-10** — P7 Track A and P8 re-pointed at [`proposals/2026-08-10-methodology-published-quote-classification-and-hierarchy.md`](../proposals/2026-08-10-methodology-published-quote-classification-and-hierarchy.md). Proposal contains the exact `/methodology` JSX insert, a "docs-only edit ⇒ 30-day notice does not apply" argument for the Committee to accept or reject, and a fallback plan if it does apply. P0 queue item 4 flipped from "write the proposal" to "review and merge the proposal, then cut the follow-up `/methodology` PR". (index-architect fourth run.)
