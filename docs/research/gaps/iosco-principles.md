...
2026-08-24 (P7 + P8 row actions repointed at the merged/pending proposal
[`proposals/2026-08-24-published-quote-benchmark-classification.md`](../proposals/2026-08-24-published-quote-benchmark-classification.md)).
...
| P7 | Data sufficiency | partial / **structurally weak** | All inputs anchored in a genuine arms-length cash market for GPU-hours; inputs are **firm executable list prices** (closer to BMR "committed quotes" than to indicative submissions). 24h rolling window, ≥ 6 providers when fully ramped. `invoice_observations` table exists (migration 011) as the latent transaction layer — empty today. | Inputs are executable **listings**, not observed trades. Strict P7 reading requires "anchored by observable transactions". On-demand compute has no public consolidated tape. Path now mapped (see note): **Track A** — self-classify as a *published-quote benchmark* + publish data-input hierarchy on `/methodology` (P0, **proposal drafted 2026-08-24** — [`proposals/2026-08-24-published-quote-benchmark-classification.md`](../proposals/2026-08-24-published-quote-benchmark-classification.md); folds in P8). **Track B** — stand up `invoice_observations` ingest, then publish a list-price-vs-observed-effective-price reconciliation report (P1, infra, no hard-limit file). **Track C** — provider-count-scaled quorum so thin-GPU indices suppress earlier (P1, methodology-class → proposal + committee). | P0 (Track A) / P1 (B, C) | Track A proposal open for Committee review: [`proposals/2026-08-24-published-quote-benchmark-classification.md`](../proposals/2026-08-24-published-quote-benchmark-classification.md). On approve+merge+effective-date, this row moves to `partial` (Tracks B, C remain). |
| P8 | Hierarchy of data inputs | partial | Hierarchy exists in code (rule → alias → fuzzy → Claude ≥ 0.95 auto → Claude 0.70–0.95 admin queue → outlier check → eligibility check → VWAP). Zero expert judgment in published-number path. | Hierarchy is implicit, not published. | P1 | Bundled into P7 Track A proposal — [`proposals/2026-08-24-published-quote-benchmark-classification.md`](../proposals/2026-08-24-published-quote-benchmark-classification.md) Section 2 is the Stage 1–5 hierarchy subsection. On merge + effective-date, this row moves to `compliant`. |
...
4. P7 + P8 — Proposal for the `/methodology` "published-quote benchmark" self-classification + data-input hierarchy subsection. **Drafted 2026-08-24 as [`proposals/2026-08-24-published-quote-benchmark-classification.md`](../proposals/2026-08-24-published-quote-benchmark-classification.md); awaiting @CarlosGalindo2807 Committee review.** Closes two quality-pillar gaps in one edit.
...
- **2026-08-24** — P7 Track A + P8 proposal drafted as [`proposals/2026-08-24-published-quote-benchmark-classification.md`](../proposals/2026-08-24-published-quote-benchmark-classification.md). Rows P7 and P8 "Owner / Action" cells repointed at the proposal; priority-queue P0 item 4 (and its P1 shadow item 7) note the proposal is open for @CarlosGalindo2807 Committee review. On merge + effective-date, P7 status moves `partial / structurally weak` → `partial` (Tracks B, C remain) and P8 moves `partial` → `compliant`. (index-architect fourth run.)
