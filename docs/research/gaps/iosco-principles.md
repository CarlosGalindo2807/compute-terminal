...
2026-05-28 (P7 + P8 action lines updated; proposal `proposals/2026-05-28-methodology-published-quote-self-classification.md` is now the in-flight artifact for both rows).
...
| P7 | Data sufficiency | partial / **structurally weak**, Track A in flight | All inputs anchored in a genuine arms-length cash market for GPU-hours; inputs are **firm executable list prices** (closer to BMR "committed quotes" than to indicative submissions). 24h rolling window, ≥ 6 providers when fully ramped. `invoice_observations` table exists (migration 011) as the latent transaction layer — empty today. | Inputs are executable **listings**, not observed trades. Strict P7 reading requires "anchored by observable transactions". On-demand compute has no public consolidated tape. Path mapped (see note): **Track A** — self-classify as a *published-quote benchmark* + publish data-input hierarchy on `/methodology` (P0; **proposal written, in review**). **Track B** — stand up `invoice_observations` ingest, then publish a list-price-vs-observed-effective-price reconciliation report (P1, infra, no hard-limit file). **Track C** — provider-count-scaled quorum so thin-GPU indices suppress earlier (P1, methodology-class → proposal + committee). | P0 (Track A) / P1 (B, C) | Note done: [`notes/2026-05-12-listings-vs-transactions-iosco-p7.md`](../notes/2026-05-12-listings-vs-transactions-iosco-p7.md). Proposal in flight: [`proposals/2026-05-28-methodology-published-quote-self-classification.md`](../proposals/2026-05-28-methodology-published-quote-self-classification.md). Next: @CarlosGalindo2807 reviews + merges the page edit; row flips to `partial / disclosed`. |
| P8 | Hierarchy of data inputs | partial, proposal in flight | Hierarchy exists in code (rule → alias → fuzzy → Claude ≥ 0.95 auto → Claude 0.70–0.95 admin queue → outlier check → eligibility check → VWAP). Zero expert judgment in published-number path. | Hierarchy is implicit, not published. | P1 | Bundled with the P7 self-classification proposal: [`proposals/2026-05-28-methodology-published-quote-self-classification.md`](../proposals/2026-05-28-methodology-published-quote-self-classification.md) §"Section 2 — Hierarchy of data inputs" publishes the deterministic six-stage hierarchy on `/methodology`. On merge, row flips to `compliant`. |
...
4. P7 + P8 — **Proposal written, awaiting @CarlosGalindo2807 review:** [`proposals/2026-05-28-methodology-published-quote-self-classification.md`](../proposals/2026-05-28-methodology-published-quote-self-classification.md). Drafts the exact `/methodology` copy for the "published-quote benchmark" self-classification + the six-stage data-input hierarchy. On merge, closes two quality-pillar gaps in one edit and flips P8 to `compliant`. Research dossier: [note](../notes/2026-05-12-listings-vs-transactions-iosco-p7.md).
...
- **2026-05-28** — P7 + P8 action lines updated: proposal [`proposals/2026-05-28-methodology-published-quote-self-classification.md`](../proposals/2026-05-28-methodology-published-quote-self-classification.md) is written and in review. Includes the exact draft copy for the `/methodology` page edit (Classification subsection + six-stage Hierarchy subsection), risk register, committee deliberation prompt, and rollout plan. No change to `PUBLISHED_METHODOLOGY` constant. Priority-queue P0 item 4 status: "proposal written, awaiting @CarlosGalindo2807 review". (index-architect fourth run.)
