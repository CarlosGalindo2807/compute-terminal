...
2026-08-13 (P7 + P8 proposal opened — `/methodology` self-classification as a
published-quote benchmark; rows updated to reflect in-flight status).
...
| P7 | Data sufficiency | partial / **proposal in flight** | All inputs anchored in a genuine arms-length cash market for GPU-hours; inputs are **firm executable list prices** (closer to BMR "committed quotes" than to indicative submissions). 24h rolling window, ≥ 6 providers when fully ramped. `invoice_observations` table exists (migration 011) as the latent transaction layer — empty today. | Inputs are executable **listings**, not observed trades. Strict P7 reading requires "anchored by observable transactions". On-demand compute has no public consolidated tape. Path now mapped (see note): **Track A** — self-classify as a *published-quote benchmark* + publish data-input hierarchy on `/methodology` (P0, **proposal open** at [`proposals/2026-08-13-methodology-p7-p8-published-quote-classification.md`](../proposals/2026-08-13-methodology-p7-p8-published-quote-classification.md); pending Committee review / merge; folds in P8, P14, P19). **Track B** — stand up `invoice_observations` ingest, then publish a list-price-vs-observed-effective-price reconciliation report (P1, infra, no hard-limit file). **Track C** — provider-count-scaled quorum so thin-GPU indices suppress earlier (P1, methodology-class → proposal + committee). | P0 (Track A) / P1 (B, C) | Proposal drafted; awaits @CarlosGalindo2807 review. On merge, status becomes `partial → self-classified` and this action item moves to "monitor for Level-1 (invoice) ingest bring-up (Track B)". |
| P8 | Hierarchy of data inputs | partial / **proposal in flight** | Hierarchy exists in code (rule → alias → fuzzy → Claude ≥ 0.95 auto → Claude 0.70–0.95 admin queue → outlier check → eligibility check → VWAP). Zero expert judgment in published-number path. | Hierarchy is implicit, not published. Bundled with the P7 self-classification in the same `/methodology` proposal — [`proposals/2026-08-13-methodology-p7-p8-published-quote-classification.md`](../proposals/2026-08-13-methodology-p7-p8-published-quote-classification.md). On merge: status becomes `compliant`. | P1 | Proposal drafted; awaits @CarlosGalindo2807 review. |
...
| P14 | Submitter Code of Conduct | n/a / **proposal in flight** | Inputs are scraped, not submitted. No Submitters in the LIBOR sense. | Non-applicability now folded into the P7/P8 `/methodology` proposal — [`proposals/2026-08-13-methodology-p7-p8-published-quote-classification.md`](../proposals/2026-08-13-methodology-p7-p8-published-quote-classification.md) §"Related principles — non-applicability". On merge: status becomes `n/a (documented)`. | — | Proposal drafted; awaits @CarlosGalindo2807 review. |
...
| P19 | Cooperation with regulatory authorities | n/a until regulated / **proposal in flight** | CTI is not an ESMA-registered or FCA-supervised benchmark today. | Non-applicability now folded into the P7/P8 `/methodology` proposal — [`proposals/2026-08-13-methodology-p7-p8-published-quote-classification.md`](../proposals/2026-08-13-methodology-p7-p8-published-quote-classification.md) §"Related principles — non-applicability". On merge: status becomes `n/a until regulated (documented)`. | — | Proposal drafted; awaits @CarlosGalindo2807 review. |
...
4. **P7 + P8 (+ P14 + P19) — Proposal in flight.** Self-classification as a *published-quote benchmark*, three-level data-input hierarchy, and n/a declarations for P14 / P19 all in the same `/methodology` edit. Proposal: [`proposals/2026-08-13-methodology-p7-p8-published-quote-classification.md`](../proposals/2026-08-13-methodology-p7-p8-published-quote-classification.md). *Pending @CarlosGalindo2807 Committee vote / PR merge. Closes four IOSCO rows in a single disclosure edit.*
...
- **2026-08-13** — P7 + P8 + P14 + P19 rows updated after [`proposals/2026-08-13-methodology-p7-p8-published-quote-classification.md`](../proposals/2026-08-13-methodology-p7-p8-published-quote-classification.md): first proposal in `proposals/` since the directory was created. Track-A recommendation from the 2026-05-12 note implemented as a docs-only edit to the hard-limit `/methodology` page — self-classify CTI as a *published-quote benchmark*, publish the three-level data-input hierarchy (Level 1 observed transactions / Level 2 firm executable quotes / Level 3 indicative), state expert-judgment non-applicability, and fold in n/a declarations for P14 (no Submitters) and P19 (not currently regulated). Rows P7, P8, P14, P19 marked *proposal in flight* pending Committee vote; the P0 priority queue entry 4 updated from "write the proposal" to "proposal open, awaits @CarlosGalindo2807 review". (index-architect fourth run.)
