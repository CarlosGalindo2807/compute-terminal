...
2026-06-01 (P7 + P8 queue items now point at a written proposal; see revision log).
...
| P7 | Data sufficiency | partial / **structurally weak** | All inputs anchored in a genuine arms-length cash market for GPU-hours; inputs are **firm executable list prices** (closer to BMR "committed quotes" than to indicative submissions). 24h rolling window, ≥ 6 providers when fully ramped. `invoice_observations` table exists (migration 011) as the latent transaction layer — empty today. | Inputs are executable **listings**, not observed trades. Strict P7 reading requires "anchored by observable transactions" — but IOSCO P7 commentary **explicitly permits** benchmarks "constructed exclusively on executable quotes" provided the methodology discloses it. Path now mapped: **Track A** — self-classify as a *published-quote benchmark* + publish data-input hierarchy on `/methodology` (**proposal written, awaiting review** — [`proposals/2026-06-01-published-quote-self-classification.md`](../proposals/2026-06-01-published-quote-self-classification.md); folds in P8). **Track B** — stand up `invoice_observations` ingest, then publish a list-price-vs-observed-effective-price reconciliation report (P1, infra, no hard-limit file). **Track C** — provider-count-scaled quorum so thin-GPU indices suppress earlier (P1, methodology-class → proposal + committee). | P0 (Track A — proposal awaiting review) / P1 (B, C) | Proposal written: [`proposals/2026-06-01-published-quote-self-classification.md`](../proposals/2026-06-01-published-quote-self-classification.md). Background note: [`notes/2026-05-12-listings-vs-transactions-iosco-p7.md`](../notes/2026-05-12-listings-vs-transactions-iosco-p7.md). Status flips to `compliant` on merge. |
| P8 | Hierarchy of data inputs | partial | Hierarchy exists in code (rule → alias → fuzzy → Claude ≥ 0.95 auto → Claude 0.70–0.95 admin queue → outlier check → eligibility check → VWAP). Zero expert judgment in published-number path. | Hierarchy is implicit, not published. | P0 (bundled with P7) | **Proposal written, awaiting review** — [`proposals/2026-06-01-published-quote-self-classification.md`](../proposals/2026-06-01-published-quote-self-classification.md) §4b adds the "Hierarchy of data inputs" subsection to `/methodology` (three-level ICE/LIBOR-style waterfall: Level 1 transaction data reserved; Level 2 executable list prices = sole present input; Level 3 expert judgment not used). Status flips to `compliant` on merge. |
...
1. P3 / P5 — Conflict-of-interest disclosure + single-administrator declaration on `/methodology`. *Half-day docs work. Natural follow-up to the P7/P8 proposal — same page, same review path.*
...
4. P7 + P8 — **Proposal written, awaiting @CarlosGalindo2807 review** ([`proposals/2026-06-01-published-quote-self-classification.md`](../proposals/2026-06-01-published-quote-self-classification.md)). Adds two subsections to `/methodology` (input data classification + hierarchy of data inputs). No computation change; no version bump; methodology lock test continues to pass. Closes two quality-pillar gaps in one edit.
...
- **2026-06-01** — P7 + P8 rows updated to point at the written proposal [`proposals/2026-06-01-published-quote-self-classification.md`](../proposals/2026-06-01-published-quote-self-classification.md), which lands Track A as docs-only edits to `/methodology` (no formula change, no version bump, lock test stays green). Both rows flip to `compliant` on merge. P8 promoted from P1 → P0 because it's bundled into the same proposal as P7. Background addition: IOSCO Principle 7 commentary explicitly permits benchmarks "constructed exclusively on executable quotes" provided the methodology discloses it — so the gap is missing disclosure, not structural deficiency. (index-architect fourth run.)
