...
2026-07-13 (P0 item 4 status: proposal drafted; P8/P14/P19 rows cross-linked to
the same proposal since they ship in one page edit).
...
| P7 | Data sufficiency | partial / **structurally weak** — Track A proposal drafted 2026-07-13 | All inputs anchored in a genuine arms-length cash market for GPU-hours; inputs are **firm executable list prices** (closer to BMR "committed quotes" than to indicative submissions). 24h rolling window, ≥ 6 providers when fully ramped. `invoice_observations` table exists (migration 011) as the latent transaction layer — empty today. | Inputs are executable **listings**, not observed trades. Strict P7 reading requires "anchored by observable transactions". On-demand compute has no public consolidated tape. Path mapped (see note): **Track A** — self-classify as a *published-quote benchmark* + publish data-input hierarchy on `/methodology` (proposal drafted [`proposals/2026-07-13-methodology-input-classification-and-hierarchy.md`](../proposals/2026-07-13-methodology-input-classification-and-hierarchy.md); awaiting @CarlosGalindo2807 review). **Track B** — stand up `invoice_observations` ingest, then publish a list-price-vs-observed-effective-price reconciliation report (P1, infra, no hard-limit file). **Track C** — provider-count-scaled quorum so thin-GPU indices suppress earlier (P1, methodology-class → proposal + committee). | P0 (Track A) / P1 (B, C) | Track A: proposal drafted, awaiting review. On merge, this row moves to `partial (Track A shipped; B, C queued)` and P0 item 4 resolves. Backing note: [`notes/2026-05-12-listings-vs-transactions-iosco-p7.md`](../notes/2026-05-12-listings-vs-transactions-iosco-p7.md). |
| P8 | Hierarchy of data inputs | partial — proposal drafted 2026-07-13 | Hierarchy exists in code (rule → alias → fuzzy → Claude ≥ 0.95 auto → Claude 0.70–0.95 admin queue → outlier check → eligibility check → VWAP). Zero expert judgment in published-number path. | Hierarchy is implicit, not published. | P1 (bundled with P7 P0) | Seven-stage hierarchy table drafted in [`proposals/2026-07-13-methodology-input-classification-and-hierarchy.md`](../proposals/2026-07-13-methodology-input-classification-and-hierarchy.md) §2a. Awaiting @CarlosGalindo2807 review. On merge this row moves to `compliant`. |
...
| P14 | Submitter Code of Conduct | n/a — disclosure drafted 2026-07-13 | Inputs are scraped, not submitted. No Submitters in the LIBOR sense. | Non-applicability not yet documented as such. | — | Non-applicability paragraph drafted in [`proposals/2026-07-13-methodology-input-classification-and-hierarchy.md`](../proposals/2026-07-13-methodology-input-classification-and-hierarchy.md) §2b. Ships in same PR. On merge this row moves to `n/a documented`. |
...
| P19 | Cooperation with regulatory authorities | n/a until regulated — disclosure drafted 2026-07-13 | CTI is not an ESMA-registered or FCA-supervised benchmark today. | Non-applicability not yet documented. | — | Non-applicability paragraph drafted in [`proposals/2026-07-13-methodology-input-classification-and-hierarchy.md`](../proposals/2026-07-13-methodology-input-classification-and-hierarchy.md) §2b. Ships in same PR. On merge this row moves to `n/a documented`. |
...
4. ~~P7 + P8~~ — **Proposal drafted 2026-07-13**: [`proposals/2026-07-13-methodology-input-classification-and-hierarchy.md`](../proposals/2026-07-13-methodology-input-classification-and-hierarchy.md). Also folds in P14 + P19 non-applicability disclosures — four principle gaps in one page edit. Awaiting @CarlosGalindo2807 review; item closes on merge of that PR.
...
12. ~~P14~~ — Bundled into the P0 item 4 proposal (2026-07-13). Awaiting review.
13. P15 — Retention policy doc; hash-chained `system_events` research note.
14. P18 — `committee-minutes/` directory + commit discipline.
15. ~~P19~~ — Bundled into the P0 item 4 proposal (2026-07-13). Awaiting review.
...
- **2026-07-13** — P0 item 4 proposal drafted: [`proposals/2026-07-13-methodology-input-classification-and-hierarchy.md`](../proposals/2026-07-13-methodology-input-classification-and-hierarchy.md). Bundles P7 (Track A self-classification), P8 (seven-stage hierarchy table), P14 (Submitter n/a), P19 (regulatory-status n/a) into one `/methodology` page edit. No `PUBLISHED_METHODOLOGY` change; no published-value change; docs-class per §6 Rollout of the proposal. Rows P7, P8, P14, P19 annotated "proposal drafted"; priority-queue items 4, 12, 15 marked bundled/awaiting review. On merge, four rows resolve. (index-architect fourth run.)
