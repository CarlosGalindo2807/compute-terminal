...
2026-07-23 (P7 + P8 queue rows now point at the pending proposal
`proposals/2026-07-23-methodology-data-inputs-hierarchy.md`).
...
| P7 | Data sufficiency | partial / **structurally weak** | All inputs anchored in a genuine arms-length cash market for GPU-hours; inputs are **firm executable list prices** (closer to BMR "committed quotes" than to indicative submissions). 24h rolling window, ≥ 6 providers when fully ramped. `invoice_observations` table exists (migration 011) as the latent transaction layer — empty today. `/markets` Δ column now suppresses % change when either endpoint has < 20 obs (commit 1e55085, 2026-05-12) — a data-sufficiency gate at the consumption surface. | Inputs are executable **listings**, not observed trades. Strict P7 reading requires "anchored by observable transactions". On-demand compute has no public consolidated tape. Path mapped: **Track A** — self-classify as a *published-quote benchmark* + publish data-input hierarchy on `/methodology`. **Proposal written** ([`proposals/2026-07-23-methodology-data-inputs-hierarchy.md`](../proposals/2026-07-23-methodology-data-inputs-hierarchy.md)); pending @CarlosGalindo2807 review + Committee decision on *disclosure clarification vs. methodology change* treatment. **Track B** — stand up `invoice_observations` ingest, then publish a list-price-vs-observed-effective-price reconciliation report (P1, infra, no hard-limit file). **Track C** — provider-count-scaled quorum so thin-GPU indices suppress earlier (P1, methodology-class → proposal + committee). | P0 (Track A — awaiting review) / P1 (B, C) | Proposal written, PR opened. On merge: disclosure sub-track closed; drop row to P1 (structural weakness remains until Track B lands). |
| P8 | Hierarchy of data inputs | partial | Hierarchy exists in code (rule → alias → fuzzy → Claude ≥ 0.95 auto → Claude 0.70–0.95 admin queue → outlier check → eligibility check → VWAP). Zero expert judgment in published-number path. | Hierarchy is implicit, not published. | P0 (bundled) | **Ships in the same proposal as P7's Track A** — [`proposals/2026-07-23-methodology-data-inputs-hierarchy.md`](../proposals/2026-07-23-methodology-data-inputs-hierarchy.md) publishes the nine-stage hierarchy table and the "no expert judgment" claim. On merge, this row closes fully to `compliant`. |
...
4. P7 + P8 — **Proposal written and PR opened**: [`proposals/2026-07-23-methodology-data-inputs-hierarchy.md`](../proposals/2026-07-23-methodology-data-inputs-hierarchy.md). Next step: @CarlosGalindo2807 review + Committee vote on *disclosure clarification vs. methodology change* treatment (the proposal recommends *clarification* — no `PUBLISHED_METHODOLOGY_VERSION` bump, `methodology_changes` row of type `disclosure_clarification`, no 30-day notice — because the published number does not move). On merge, closes P8 fully and closes the disclosure sub-track of P7 (P7's structural weakness persists until `invoice_observations` Track B lands).
...
7. ~~P8 — Hierarchy-of-data-inputs subsection on `/methodology`~~ (now fully bundled into P0 item 4; row moves to `compliant` on merge of that proposal — this line stays for one revision so the change is legible, then gets removed).
...
- **2026-07-23** — P7 + P8 rows point at the pending proposal [`proposals/2026-07-23-methodology-data-inputs-hierarchy.md`](../proposals/2026-07-23-methodology-data-inputs-hierarchy.md). P7 row also acknowledges the 2026-05-12 `/markets` Δ suppression gate (commit 1e55085) as a consumption-surface data-sufficiency control. P0 queue item 4 progresses from "write the proposal" to "review and merge the proposal"; the deliberation ask is the *disclosure-clarification vs. methodology-change* treatment decision, which sets precedent for future clarifying docs edits to hard-limit surfaces. On merge P8 fully closes; P7's disclosure sub-track closes but structural weakness persists until Track B (invoice-observation ingest) lands. (index-architect fourth run.)
