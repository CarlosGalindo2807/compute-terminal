...
2026-07-02 (P7/P8 rows + queue updated after the self-classification proposal).
...
| P7 | Data sufficiency | partial / **structurally weak** — Track A **proposal drafted 2026-07-02** | All inputs anchored in a genuine arms-length cash market for GPU-hours; inputs are **firm executable list prices** (closer to BMR "committed quotes" than to indicative submissions). 24h rolling window, ≥ 6 providers when fully ramped. `invoice_observations` table exists (migration 011) as the latent transaction layer — empty today. | Inputs are executable **listings**, not observed trades. Strict P7 reading requires "anchored by observable transactions". On-demand compute has no public consolidated tape. Path now mapped (see note): **Track A — proposal drafted, pending Committee review** — self-classify as a *published-quote benchmark* + publish data-input hierarchy on `/methodology` (P0, `/methodology` is hard-limit; folds in P8). **Track B** — stand up `invoice_observations` ingest, then publish a list-price-vs-observed-effective-price reconciliation report (P1, infra, no hard-limit file). **Track C** — provider-count-scaled quorum so thin-GPU indices suppress earlier (P1, methodology-class → proposal + committee). | P0 (Track A) / P1 (B, C) | Note done: [`notes/2026-05-12-listings-vs-transactions-iosco-p7.md`](../notes/2026-05-12-listings-vs-transactions-iosco-p7.md). Proposal done: [`proposals/2026-07-02-methodology-p7-p8-self-classification.md`](../proposals/2026-07-02-methodology-p7-p8-self-classification.md). Next: Committee sign-off + PR merge; then Track B (invoice-observation reconciliation). |
| P8 | Hierarchy of data inputs | partial — **proposal drafted 2026-07-02** | Hierarchy exists in code (rule → alias → fuzzy → Claude ≥ 0.95 auto → Claude 0.70–0.95 admin queue → outlier check → eligibility check → VWAP). Zero expert judgment in published-number path. | Hierarchy is implicit, not published. Closed on merge of the P7 + P8 proposal — same edit to the same page. | P1 | Proposal drafted: [`proposals/2026-07-02-methodology-p7-p8-self-classification.md`](../proposals/2026-07-02-methodology-p7-p8-self-classification.md) §Proposed change (subsection 2). Awaiting Committee sign-off. |
...
4. P7 + P8 — **Proposal drafted 2026-07-02** ([`proposals/2026-07-02-methodology-p7-p8-self-classification.md`](../proposals/2026-07-02-methodology-p7-p8-self-classification.md)). *Awaiting Committee sign-off. On approval, one PR edits the hard-limit `/methodology` page with the two new subsections; closes two quality-pillar gaps in one edit.*
...
- **2026-07-02** — Track A proposal drafted: [`proposals/2026-07-02-methodology-p7-p8-self-classification.md`](../proposals/2026-07-02-methodology-p7-p8-self-classification.md). P7 status updated to "Track A proposal drafted, pending Committee review"; P8 similarly annotated. Priority-queue P0 item 4 updated from "write the proposal" to "Committee sign-off + PR merge". Neither row's `partial`/`gap` classification changes until the PR editing `/methodology` merges. (index-architect fourth run.)
