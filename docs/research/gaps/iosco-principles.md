...
2026-08-17 (P7 + P8 rows point at the new proposal; P0 queue item 4 advanced from
"write the proposal" to "await Committee review of the drafted proposal").
...
| P7 | Data sufficiency | partial / **structurally weak** (Track A proposal drafted, pending Committee) | All inputs anchored in a genuine arms-length cash market for GPU-hours; inputs are **firm executable list prices** (closer to BMR "committed quotes" than to indicative submissions). 24h rolling window, ≥ 6 providers when fully ramped. `invoice_observations` table exists (migration 011) as the latent transaction layer — empty today. | Inputs are executable **listings**, not observed trades. Strict P7 reading requires "anchored by observable transactions". On-demand compute has no public consolidated tape. Path mapped (see note): **Track A** — self-classify as a *published-quote benchmark* + publish data-input hierarchy on `/methodology` (P0, hard-limit surface — proposal now drafted at [`proposals/2026-08-17-published-quote-benchmark-self-classification.md`](../proposals/2026-08-17-published-quote-benchmark-self-classification.md), folds in P8). **Track B** — stand up `invoice_observations` ingest, then publish a list-price-vs-observed-effective-price reconciliation report (P1, infra, no hard-limit file). **Track C** — provider-count-scaled quorum so thin-GPU indices suppress earlier (P1, methodology-class → proposal + committee). | P0 (Track A) / P1 (B, C) | Track A proposal awaiting @CarlosGalindo2807 Committee review. On approval, `methodology_changes` row + follow-up PR editing `apps/web/app/methodology/page.tsx` scheduled for 2026-09-17 effective date (30-day notice). |
| P8 | Hierarchy of data inputs | partial (proposal drafted, pending Committee) | Hierarchy exists in code (rule → alias → fuzzy → Claude ≥ 0.95 auto → Claude 0.70–0.95 admin queue → outlier check → eligibility check → VWAP). Zero expert judgment in published-number path. | Hierarchy is implicit, not published. | P1 | Bundled into the P7 Track A proposal: [`proposals/2026-08-17-published-quote-benchmark-self-classification.md`](../proposals/2026-08-17-published-quote-benchmark-self-classification.md) §2.2 adds the "Data-input hierarchy" subsection verbatim. Awaiting Committee review; follow-up PR to `apps/web/app/methodology/page.tsx` scheduled for 2026-09-17. |
...
4. P7 + P8 — Proposal for the `/methodology` "published-quote benchmark" self-classification + data-input hierarchy subsection. *Proposal drafted 2026-08-17 at [`proposals/2026-08-17-published-quote-benchmark-self-classification.md`](../proposals/2026-08-17-published-quote-benchmark-self-classification.md), awaiting @CarlosGalindo2807 Committee review. On approval → `methodology_changes` row + follow-up PR editing `apps/web/app/methodology/page.tsx`, effective 2026-09-17 (30-day notice, also serves as the P12 change-procedure dry run). Closes two quality-pillar gaps in one edit.*
...
- **2026-08-17** — P7 Track A proposal drafted at [`proposals/2026-08-17-published-quote-benchmark-self-classification.md`](../proposals/2026-08-17-published-quote-benchmark-self-classification.md), covering both P7 self-classification and P8 hierarchy publication in one edit to `/methodology`. P7 and P8 rows updated to "proposal drafted, pending Committee". P0 queue item 4 advanced from "write the proposal" to "await Committee review; follow-up PR + `methodology_changes` row scheduled for 2026-09-17 effective date (30-day notice, doubles as the P12 change-procedure dry run)". No numerical or code change in this revision. (index-architect fourth run.)
