...
2026-06-11 (P7 + P8 action items now reference the proposal in `proposals/`).
...
| P7 | Data sufficiency | partial / **structurally weak**; Track A proposal authored 2026-06-11 awaiting Committee approval | All inputs anchored in a genuine arms-length cash market for GPU-hours; inputs are **firm executable list prices** (closer to BMR "committed quotes" than to indicative submissions). 24h rolling window, ≥ 6 providers when fully ramped. `invoice_observations` table exists (migration 011) as the latent transaction layer — empty today. | Inputs are executable **listings**, not observed trades. Strict P7 reading requires "anchored by observable transactions". On-demand compute has no public consolidated tape. Path now mapped (see note): **Track A** — self-classify as a *published-quote benchmark* + publish data-input hierarchy on `/methodology` (P0, needs a proposal — `/methodology` is hard-limit; folds in P8). **Track B** — stand up `invoice_observations` ingest, then publish a list-price-vs-observed-effective-price reconciliation report (P1, infra, no hard-limit file). **Track C** — provider-count-scaled quorum so thin-GPU indices suppress earlier (P1, methodology-class → proposal + committee). | P0 (Track A) / P1 (B, C) | Note done: [`notes/2026-05-12-listings-vs-transactions-iosco-p7.md`](../notes/2026-05-12-listings-vs-transactions-iosco-p7.md). **Proposal authored 2026-06-11**: [`proposals/2026-06-11-published-quote-benchmark-classification.md`](../proposals/2026-06-11-published-quote-benchmark-classification.md) — awaiting @CarlosGalindo2807 review; follow-up `/methodology` page-edit PR is gated on this approval. |
| P8 | Hierarchy of data inputs | partial; bundled into P7 Track A proposal authored 2026-06-11 | Hierarchy exists in code (rule → alias → fuzzy → Claude ≥ 0.95 auto → Claude 0.70–0.95 admin queue → outlier check → eligibility check → VWAP). Zero expert judgment in published-number path. | Hierarchy is implicit, not published. | P1 | Add "Hierarchy of data inputs" subsection to `/methodology` listing each ingestion stage and the deterministic rule — **bundled with the P7 self-classification** in the same `/methodology` proposal: [`proposals/2026-06-11-published-quote-benchmark-classification.md`](../proposals/2026-06-11-published-quote-benchmark-classification.md) (Edit 2). |
...
4. P7 + P8 — Proposal for the `/methodology` "published-quote benchmark" self-classification + data-input hierarchy subsection. **Proposal authored 2026-06-11** ([`proposals/2026-06-11-published-quote-benchmark-classification.md`](../proposals/2026-06-11-published-quote-benchmark-classification.md)); awaiting @CarlosGalindo2807 review. Follow-up `/methodology` page-edit PR is gated on this approval. Closes two quality-pillar gaps in one edit.
...
- **2026-06-11** — P7 + P8 proposal authored: [`proposals/2026-06-11-published-quote-benchmark-classification.md`](../proposals/2026-06-11-published-quote-benchmark-classification.md). Rows P7 and P8 statuses appended with "Track A proposal authored 2026-06-11 awaiting Committee approval". Priority-queue P0 item 4 updated to reflect proposal-authored state. Proposal recommends a two-PR sequence (this proposal PR → follow-up `/methodology` page-edit PR) and triggers no 30-day public-notice period because the published number, formula, and inputs are unchanged. Next-session candidate: P3 / P5 — conflict-of-interest disclosure + single-administrator declaration. (index-architect fourth run.)
