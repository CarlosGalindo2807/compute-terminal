...
2026-09-10 (P7/P8 rows + queue updated after the published-quote-benchmark
proposal landed).
...
| P7 | Data sufficiency | partial / **structurally weak** | All inputs anchored in a genuine arms-length cash market for GPU-hours; inputs are **firm executable list prices** (closer to BMR "committed quotes" than to indicative submissions). 24h rolling window, ≥ 6 providers when fully ramped. `invoice_observations` table exists (migration 011) as the latent transaction layer — empty today. | Inputs are executable **listings**, not observed trades. Strict P7 reading requires "anchored by observable transactions". On-demand compute has no public consolidated tape. Three-track response now in flight: **Track A** — self-classify as a *published-quote benchmark* + publish data-input hierarchy on `/methodology` — **proposal drafted** at [`proposals/2026-09-10-published-quote-benchmark-and-data-input-hierarchy.md`](../proposals/2026-09-10-published-quote-benchmark-and-data-input-hierarchy.md); pending Committee review + implementation PR + 30-day notice. Closes the disclosure half of P7 and all of P8 in one edit. **Track B** — stand up `invoice_observations` ingest, then publish a list-price-vs-observed-effective-price reconciliation report (P1, infra, no hard-limit file). Becomes a stated forward obligation once Track A ships. **Track C** — provider-count-scaled quorum so thin-GPU indices suppress earlier (P1, methodology-class → proposal + committee). | P0 (Track A — in review) / P1 (B, C) | Proposal in review: [`proposals/2026-09-10-published-quote-benchmark-and-data-input-hierarchy.md`](../proposals/2026-09-10-published-quote-benchmark-and-data-input-hierarchy.md). On merge + effective date (~2026-10-11), status becomes `partial (disclosed; anchor deferred to Track B)`. |
| P8 | Hierarchy of data inputs | partial | Hierarchy exists in code (rule → alias → fuzzy → Claude ≥ 0.95 auto → Claude 0.70–0.95 admin queue → outlier check → eligibility check → VWAP). Zero expert judgment in published-number path. | Hierarchy is implicit, not published. | P0 (bundled with P7 Track A) | Bundled into the P7 proposal at [`proposals/2026-09-10-published-quote-benchmark-and-data-input-hierarchy.md`](../proposals/2026-09-10-published-quote-benchmark-and-data-input-hierarchy.md) — one new "Data inputs" section on `/methodology` publishes the 10-step hierarchy verbatim and explicitly claims "no expert judgment in the published-number path". On merge + effective date, status becomes `compliant`. |
...
4. ~~P7 + P8 — Proposal for the `/methodology` "published-quote benchmark" self-classification + data-input hierarchy subsection.~~ **Proposal in review** ([`proposals/2026-09-10-published-quote-benchmark-and-data-input-hierarchy.md`](../proposals/2026-09-10-published-quote-benchmark-and-data-input-hierarchy.md)). On Committee approval, opens follow-up implementation PR against `apps/web/app/methodology/page.tsx` with 30-day notice banner; effective ~2026-10-11.
...
7. ~~P8 — Hierarchy-of-data-inputs subsection on `/methodology`.~~ Bundled into P0 item 4 (proposal in review).
...
- **2026-09-10** — P7 Track A proposal drafted at [`proposals/2026-09-10-published-quote-benchmark-and-data-input-hierarchy.md`](../proposals/2026-09-10-published-quote-benchmark-and-data-input-hierarchy.md); it self-classifies CTI as a "published-quote benchmark" and adds a "Data inputs" section (Benchmark classification / Hierarchy of data inputs / Expert judgment) to `/methodology`. `PUBLISHED_METHODOLOGY` constant and every computed value are unchanged. P7 and P8 rows updated to point at the proposal; P0-queue item 4 struck through pending Committee approval; P1-queue item 7 struck through (bundled). Status transitions on merge + effective date (~2026-10-11): P7 → `partial (disclosed; anchor deferred to Track B)`, P8 → `compliant`. (index-architect fourth run.)
