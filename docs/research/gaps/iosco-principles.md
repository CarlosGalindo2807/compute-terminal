...
2026-09-14 (P7+P8 proposal drafted — priority-queue row 4 moves from "write the proposal" to "proposal in review").
...
| P7 | Data sufficiency | partial / **structurally weak** (path to `partial / defensible` on merge of the pending proposal) | All inputs anchored in a genuine arms-length cash market for GPU-hours; inputs are **firm executable list prices** (closer to BMR "committed quotes" than to indicative submissions). 24h rolling window, ≥ 6 providers when fully ramped. `invoice_observations` table exists (migration 011) as the latent transaction layer — empty today. | Inputs are executable **listings**, not observed trades. Strict P7 reading requires "anchored by observable transactions". On-demand compute has no public consolidated tape. Path now mapped (see note): **Track A** — self-classify as a *published-quote benchmark* + publish data-input hierarchy on `/methodology` (P0, needs a proposal — `/methodology` is hard-limit; folds in P8). **Track B** — stand up `invoice_observations` ingest, then publish a list-price-vs-observed-effective-price reconciliation report (P1, infra, no hard-limit file). **Track C** — provider-count-scaled quorum so thin-GPU indices suppress earlier (P1, methodology-class → proposal + committee). | P0 (Track A) / P1 (B, C) | **Proposal in review:** [`proposals/2026-09-14-p7-p8-published-quote-self-classification.md`](../proposals/2026-09-14-p7-p8-published-quote-self-classification.md) (@CarlosGalindo2807). Foundational note: [`notes/2026-05-12-listings-vs-transactions-iosco-p7.md`](../notes/2026-05-12-listings-vs-transactions-iosco-p7.md). |
| P8 | Hierarchy of data inputs | partial (path to `compliant` on merge of the pending proposal) | Hierarchy exists in code (rule → alias → fuzzy → Claude ≥ 0.95 auto → Claude 0.70–0.95 admin queue → outlier check → eligibility check → VWAP). Zero expert judgment in published-number path. | Hierarchy is implicit, not published. | P1 (bundled with P7 in the pending P0 proposal) | **Proposal in review:** [`proposals/2026-09-14-p7-p8-published-quote-self-classification.md`](../proposals/2026-09-14-p7-p8-published-quote-self-classification.md) §Section 2 documents the seven-stage hierarchy as proposed `/methodology` markup. Ships in the same PR as the P7 self-classification. |
...
4. P7 + P8 — Proposal for the `/methodology` "published-quote benchmark" self-classification + data-input hierarchy subsection. *Proposal drafted 2026-09-14: [`proposals/2026-09-14-p7-p8-published-quote-self-classification.md`](../proposals/2026-09-14-p7-p8-published-quote-self-classification.md). Awaiting @CarlosGalindo2807 review; on approval, a follow-up `docs:`-prefixed PR copies the two `<section>` blocks into `apps/web/app/methodology/page.tsx`. Closes two quality-pillar gaps in one edit (P7 → `partial (defensible)`, P8 → `compliant`), plus documents P14 and P19 as reasoned n/a.*
...
- **2026-09-14** — Priority-queue P0 item 4 status update: proposal now drafted at [`proposals/2026-09-14-p7-p8-published-quote-self-classification.md`](../proposals/2026-09-14-p7-p8-published-quote-self-classification.md), awaiting @CarlosGalindo2807 review. Proposal quotes IOSCO FR07/13 P7 verbatim (including the executable-quote carve-out) and BMR Art. 3(1)(14), 11(1)(c), 11(3)(d) verbatim; the same edit is scoped to document P14 and P19 as reasoned non-applicability. No hard-limit file touched by this drafting step. (index-architect fourth run.)
