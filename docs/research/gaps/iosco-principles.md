...
2026-08-06 (P7 + P8 rows updated after the published-quote-self-classification
...
| P7 | Data sufficiency | partial — **disclosure proposal drafted** | All inputs anchored in a genuine arms-length cash market for GPU-hours; inputs are **firm executable list prices** (closer to BMR "committed quotes" than to indicative submissions). 24h rolling window, ≥ 6 providers when fully ramped. `invoice_observations` table exists (migration 011) as the latent transaction layer — empty today. **Proposal to self-classify as "published-quote benchmark" per BMR Art 11(1)(c) drafted 2026-08-06** — awaiting Committee sign-off + 30-day notice + page edit. | Track A now in flight as [`proposals/2026-08-06-methodology-published-quote-self-classification.md`](../proposals/2026-08-06-methodology-published-quote-self-classification.md). Once merged + page edit lands (day 30), disclosure gap on P7 closes; **transaction-anchoring** remains a P1 infra item (Track B — `invoice_observations` ingest → reconciliation report). Track C (provider-count-scaled quorum) remains a P1 methodology-class item requiring a separate future proposal. | P0 (Track A — in flight) / P1 (B, C) | Awaiting Committee review of the drafted proposal. On approval, follow-up PR on day 30 edits `apps/web/app/methodology/page.tsx`. Track B blocked on `invoice_observations` ingest pipeline (out of index-architect scope). |
| P8 | Hierarchy of data inputs | partial — **disclosure proposal drafted** | Hierarchy exists in code (rule → alias → fuzzy → Claude ≥ 0.95 auto → Claude 0.70–0.95 admin queue → outlier check → eligibility check → VWAP). Zero expert judgment in published-number path. **Full 7-step hierarchy for `/methodology` publication drafted 2026-08-06** — bundled into the same proposal as the P7 self-classification. | Hierarchy is implicit in code, awaiting Committee sign-off + 30-day notice + page edit before it becomes explicit on the published spec. See [`proposals/2026-08-06-methodology-published-quote-self-classification.md`](../proposals/2026-08-06-methodology-published-quote-self-classification.md) §"Subsection 2 — Hierarchy". | P1 (in flight with P7) | Awaiting Committee review of the drafted proposal — same edit to `apps/web/app/methodology/page.tsx` as P7. |
...
4. P7 + P8 — **In flight.** Proposal for `/methodology` "published-quote benchmark" self-classification + data-input hierarchy drafted 2026-08-06: [`proposals/2026-08-06-methodology-published-quote-self-classification.md`](../proposals/2026-08-06-methodology-published-quote-self-classification.md). Awaiting Committee approval + 30-day public notice; page edit follows on day 30 (effective 2026-09-05).
...
- **2026-08-06** — P7 and P8 rows moved to `partial — disclosure proposal drafted`. The Track A proposal now exists at [`proposals/2026-08-06-methodology-published-quote-self-classification.md`](../proposals/2026-08-06-methodology-published-quote-self-classification.md), quotes verbatim IOSCO P7 / P8 and BMR Art 11(1)(c)/11(3)(d) text (retrieved via WebSearch snippets — direct PDF fetch still 403), and specifies the two subsections to add to `/methodology` under a 30-day notice track. Priority-queue P0 item 4 updated from "write the proposal" to "in flight — awaiting Committee approval". (index-architect fourth run.)
