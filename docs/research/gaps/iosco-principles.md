...
2026-05-14 (P7 + P8 rows pointed at the merged-pending self-classification proposal;
queue P0 #4 advances from "write the proposal" to "merge it").
...
| P7 | Data sufficiency | partial / **structurally weak**, **closure pending proposal merge** | All inputs anchored in a genuine arms-length cash market for GPU-hours; inputs are **firm executable list prices** (closer to BMR "committed quotes" than to indicative submissions). 24h rolling window, ≥ 6 providers when fully ramped. `invoice_observations` table exists (migration 011) as the latent transaction layer — empty today. | Inputs are executable **listings**, not observed trades. Strict P7 reading requires "anchored by observable transactions". On-demand compute has no public consolidated tape. Path now in flight: **Track A** — self-classify as a *published-quote benchmark* + publish data-input hierarchy on `/methodology` (P0, **proposal drafted** — [`proposals/2026-05-14-methodology-self-classification-and-input-hierarchy.md`](../proposals/2026-05-14-methodology-self-classification-and-input-hierarchy.md); folds in P8 in the same page edit). **Track B** — stand up `invoice_observations` ingest, then publish a list-price-vs-observed-effective-price reconciliation report (P1, infra, no hard-limit file). **Track C** — provider-count-scaled quorum so thin-GPU indices suppress earlier (P1, methodology-class → proposal + committee). | P0 (Track A) / P1 (B, C) | Proposal drafted ([`proposals/2026-05-14-methodology-self-classification-and-input-hierarchy.md`](../proposals/2026-05-14-methodology-self-classification-and-input-hierarchy.md)). Next: @CarlosGalindo2807 reviewer decision; on merge, row flips to `compliant (claim defensible)`. |
| P8 | Hierarchy of data inputs | partial, **closure pending proposal merge** | Hierarchy exists in code (rule → alias → fuzzy → Claude ≥ 0.95 auto → Claude 0.70–0.95 admin queue → outlier check → eligibility check → VWAP). Zero expert judgment in published-number path. | Hierarchy is implicit, not published. | P1 (bundled with P0 P7 proposal) | Hierarchy subsection drafted as part of the P7 proposal — [`proposals/2026-05-14-methodology-self-classification-and-input-hierarchy.md`](../proposals/2026-05-14-methodology-self-classification-and-input-hierarchy.md) §2 "Hierarchy of data inputs" insertion. Same edit to `/methodology` ships both rows simultaneously. |
...
1. P3 / P5 — Conflict-of-interest disclosure + single-administrator declaration on `/methodology`. *Half-day docs work.* **(Now the next-session priority.)**
...
4. P7 + P8 — **Proposal drafted, awaiting review.** [`proposals/2026-05-14-methodology-self-classification-and-input-hierarchy.md`](../proposals/2026-05-14-methodology-self-classification-and-input-hierarchy.md). Inserts two prose sections on `/methodology` (classification + hierarchy); zero methodology change; closes P7 and P8 in one edit on merge. Next step is @CarlosGalindo2807 reviewer decision — not new work.
...
- **2026-05-14** — P7 + P8 closure-pending the merged proposal [`proposals/2026-05-14-methodology-self-classification-and-input-hierarchy.md`](../proposals/2026-05-14-methodology-self-classification-and-input-hierarchy.md). Both rows mark "closure pending proposal merge"; priority-queue P0 #4 advances from "write the proposal" to "@CarlosGalindo2807 reviewer decision". P0 #1 (P3/P5 single-administrator + COI disclosure) is the next-session priority. (index-architect fourth run.)
