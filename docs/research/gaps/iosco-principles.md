...
2026-09-17 (P7 / P8 draft proposal linked; P0 item 4 now waiting on Committee
review rather than on drafting).
...
| P7 | Data sufficiency | partial / **structurally weak** (Track A proposal drafted, awaiting Committee) | All inputs anchored in a genuine arms-length cash market for GPU-hours; inputs are **firm executable list prices** (closer to BMR "committed quotes" than to indicative submissions). 24h rolling window, ≥ 6 providers when fully ramped. `invoice_observations` table exists (migration 011) as the latent transaction layer — empty today. | Inputs are executable **listings**, not observed trades. Strict P7 reading requires "anchored by observable transactions". On-demand compute has no public consolidated tape. Path now mapped (see note): **Track A** — self-classify as a *published-quote benchmark* + publish data-input hierarchy on `/methodology` (P0, **proposal drafted 2026-09-17** — `/methodology` is hard-limit; folds in P8). **Track B** — stand up `invoice_observations` ingest, then publish a list-price-vs-observed-effective-price reconciliation report (P1, infra, no hard-limit file). **Track C** — provider-count-scaled quorum so thin-GPU indices suppress earlier (P1, methodology-class → proposal + committee). | P0 (Track A — Committee review) / P1 (B, C) | Note done: [`notes/2026-05-12-listings-vs-transactions-iosco-p7.md`](../notes/2026-05-12-listings-vs-transactions-iosco-p7.md). Proposal awaiting review: [`proposals/2026-09-17-published-quote-benchmark-classification.md`](../proposals/2026-09-17-published-quote-benchmark-classification.md). Next: Committee decision + follow-up PR editing `/methodology`. |
| P8 | Hierarchy of data inputs | partial (bundled proposal drafted, awaiting Committee) | Hierarchy exists in code (rule → alias → fuzzy → Claude ≥ 0.95 auto → Claude 0.70–0.95 admin queue → outlier check → eligibility check → VWAP). Zero expert judgment in published-number path. | Hierarchy is implicit, not published. | P0 (bundled with P7) | Add "Hierarchy of data inputs" subsection to `/methodology` listing each ingestion stage and the deterministic rule — **shipped in the same proposal as the P7 self-classification** (same edit to the same page). Proposal: [`proposals/2026-09-17-published-quote-benchmark-classification.md`](../proposals/2026-09-17-published-quote-benchmark-classification.md). See [`notes/2026-05-12-listings-vs-transactions-iosco-p7.md`](../notes/2026-05-12-listings-vs-transactions-iosco-p7.md) §4–5. |
...
4. P7 + P8 — Proposal for the `/methodology` "published-quote benchmark" self-classification + data-input hierarchy subsection. *Research done ([note](../notes/2026-05-12-listings-vs-transactions-iosco-p7.md)); **proposal drafted 2026-09-17** ([`proposals/2026-09-17-published-quote-benchmark-classification.md`](../proposals/2026-09-17-published-quote-benchmark-classification.md)) — awaiting Committee review. Next step after approval: follow-up PR editing the hard-limit `/methodology` page verbatim per the proposal. Closes two quality-pillar gaps in one edit.*
...
- **2026-09-17** — P7 + P8 proposal drafted at [`proposals/2026-09-17-published-quote-benchmark-classification.md`](../proposals/2026-09-17-published-quote-benchmark-classification.md). Row P7 status annotated "Track A proposal drafted, awaiting Committee"; row P8 promoted from P1 to P0 as bundled with P7. Priority-queue P0 item 4 changed from "write the proposal" to "Committee decision on the drafted proposal". No numerical impact on `PUBLISHED_METHODOLOGY`; proposal is docs-adjacent to the hard-limit `/methodology` page. (index-architect fourth run.)
