...
2026-07-16 (P7 and P8 rows now point at the published-quote self-classification
proposal; queue P0 item 4 downgraded from "write the proposal" to
"land the implementation PR after proposal review").
...
| P7 | Data sufficiency | partial / **structurally weak** | All inputs anchored in a genuine arms-length cash market for GPU-hours; inputs are **firm executable list prices** (closer to BMR "committed quotes" than to indicative submissions). 24h rolling window, ≥ 6 providers when fully ramped. `invoice_observations` table exists (migration 011) as the latent transaction layer — empty today. | Inputs are executable **listings**, not observed trades. Strict P7 reading requires "anchored by observable transactions". On-demand compute has no public consolidated tape. Path mapped: **Track A** — self-classify as a *published-quote benchmark* + publish data-input hierarchy on `/methodology`. Proposal drafted (2026-07-16); awaiting @CarlosGalindo2807 review, then implementation PR against the hard-limit page. **Track B** — stand up `invoice_observations` ingest, then publish a list-price-vs-observed-effective-price reconciliation report (P1, infra, no hard-limit file). **Track C** — provider-count-scaled quorum so thin-GPU indices suppress earlier (P1, methodology-class → proposal + committee). | P0 (Track A) / P1 (B, C) | Proposal: [`proposals/2026-07-16-methodology-published-quote-self-classification.md`](../proposals/2026-07-16-methodology-published-quote-self-classification.md). Companion note: [`notes/2026-05-12-listings-vs-transactions-iosco-p7.md`](../notes/2026-05-12-listings-vs-transactions-iosco-p7.md). Next: land implementation PR after proposal review. |
| P8 | Hierarchy of data inputs | partial | Hierarchy exists in code (rule → alias → fuzzy → Claude ≥ 0.95 auto → Claude 0.70–0.95 admin queue → outlier check → eligibility check → VWAP). Zero expert judgment in published-number path. | Hierarchy is implicit, not published. | P1 | Bundled into the P7 proposal above — Stages 1–6 of the hierarchy are the second subsection of the proposed `/methodology` "Input data" section. Row flips to `compliant` on merge of the implementation PR. See [`proposals/2026-07-16-methodology-published-quote-self-classification.md`](../proposals/2026-07-16-methodology-published-quote-self-classification.md) and [`notes/2026-05-12-listings-vs-transactions-iosco-p7.md`](../notes/2026-05-12-listings-vs-transactions-iosco-p7.md) §4–5. |
...
4. P7 + P8 — Land the implementation PR against `apps/web/app/methodology/page.tsx` following the [2026-07-16 proposal](../proposals/2026-07-16-methodology-published-quote-self-classification.md). *Research done ([note](../notes/2026-05-12-listings-vs-transactions-iosco-p7.md)) and proposal drafted; next step is @CarlosGalindo2807 review of the proposal, then a separate implementation PR that adds the "Input data" section (classification + Stages 1–6 hierarchy + non-transaction anchoring) between the existing Formula and Index Committee sections. Closes P8 outright and closes P7 Track A.*
...
- **2026-07-16** — Proposal [`proposals/2026-07-16-methodology-published-quote-self-classification.md`](../proposals/2026-07-16-methodology-published-quote-self-classification.md) drafted and PR opened. P7 and P8 evidence cells now reference the proposal; P0 item 4 in the priority queue downgraded from "write the proposal" to "land the implementation PR after @CarlosGalindo2807 review of the proposal". Rows remain `partial` / `partial` until the implementation PR merges — the proposal PR is docs only. (index-architect fourth run.)
